...
DEFAULT_IMAGE_PATH = "images/{image_key}/episode-{episode_index:06d}/frame_{frame_index:06d}.jpeg"
...
def is_valid_numpy_dtype_string(dtype_str: str) -> bool:
    """Return whether ``dtype_str`` names a NumPy dtype."""
    try:
        np.dtype(dtype_str)
        return True
    except TypeError:
...
def check_datasets_dtype(table):
    existing_metadata = table.schema.metadata
    hf_meta_bytes = existing_metadata.get(b"huggingface")

    if hf_meta_bytes:
        hf_meta_json = json.loads(hf_meta_bytes.decode("utf-8"))
        features = hf_meta_json["info"]["features"]

        for col_name, feat_spec in features.items():
            if isinstance(feat_spec, dict) and feat_spec.get("_type") == "List":
                feat_spec["_type"] = "Sequence"
        new_hf_meta_json = json.dumps(hf_meta_json)

        new_metadata = existing_metadata.copy()
        new_metadata[b"huggingface"] = new_hf_meta_json.encode("utf-8")
        fixed_table = table.replace_schema_metadata(new_metadata)
        return fixed_table
    return table
...
def load_nested_dataset(
    ...
    Args:
        ...
        in_memory: If True, load data into RAM via PyArrow instead of memory-mapping.
            Eliminates NAS random I/O at the cost of startup time and RAM usage.
    """
    ...
    if episodes is None:
        if in_memory:
            # Load into RAM via PyArrow (bypasses mmap for NAS-bound workloads)
            table = pa_ds.dataset(
                [str(p) for p in paths], format="parquet"
            ).to_table(columns=columns)
            if table.schema.metadata:
                table = check_datasets_dtype(table)
            dataset = Dataset(table)
            dataset_name = pq_dir.parent.name or str(pq_dir.parent)
            logging.getLogger(__name__).info(
                f"[in_memory] Loaded {dataset_name}: "
                f"{len(dataset)} rows, {table.nbytes / 1024**2:.1f} MB in RAM"
            )
            return dataset
        ...
        dataset_name = pq_dir.parent.name or str(pq_dir.parent)
        with _patch_hf_generating_tqdm(dataset_name):
            return Dataset.from_parquet([str(path) for path in paths], **kwargs)
    ...
    # write a wrapper to support higher-version lerobot datasets
    table = check_datasets_dtype(table)
...
def _patch_hf_generating_tqdm(dataset_name: str):
    ...
    base_tqdm = datasets_builder.hf_tqdm

    class SingleLineGeneratingTqdm(base_tqdm):
        def __init__(self, *args, **kwargs):
            desc = kwargs.get("desc")
            if isinstance(desc, str) and desc.startswith("Generating "):
                kwargs["desc"] = f"Generating {dataset_name}"
    ...
    with patch.object(datasets_builder, "hf_tqdm", SingleLineGeneratingTqdm):
        yield
...
def load_stats(local_dir: Path) -> dict[str, dict[str, np.ndarray]] | None:
    ...
    # Fix legacy image stats: some datasets store image stats as (3,) instead of (3,1,1).
    # Reshape to (3,1,1) so aggregate_stats validation passes.
    for fkey in stats:
        if fkey.startswith("observation.images."):
            for k, v in stats[fkey].items():
                if k != "count" and isinstance(v, np.ndarray) and v.shape == (3,):
                    stats[fkey][k] = v.reshape(3, 1, 1)
    return stats
