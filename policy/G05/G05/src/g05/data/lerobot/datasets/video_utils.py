...
def get_safe_default_codec():
    if importlib.util.find_spec("torchcodec"):
        try:
            ...
            logging.warning(
                "'torchcodec' is installed but cannot be loaded (%s); falling back to 'pyav'",
                exc,
            )
...
def decode_video_frames_torchcodec(
    ...
    # Use frame_indices / fps (float64) instead of raw timestamps (float32 from parquet).
    # At large timestamps (~32000s), float32 precision ~2ms >> tolerance 0.333ms,
    # causing false assertion failures even when the correct frame was loaded.
    query_ts = torch.tensor([idx / average_fps for idx in frame_indices], dtype=torch.float64)
...
def get_video_frame_count(video_path: Path | str) -> int:
    """
    Get the number of frames in a video file using PyAV stream metadata.

    Reads only stream headers (fast). Falls back to packet counting if metadata
    is unavailable.

    Note: prefer this over get_video_duration_in_s for timestamp computation,
    as frame count avoids floating-point drift from accumulating per-episode durations.
    """
    with av.open(str(video_path)) as container:
        video_stream = container.streams.video[0]
        if video_stream.frames:
            return int(video_stream.frames)
        # Fallback: count packets with valid PTS (slower, reads full file)
        count = 0
...
def concatenate_video_files(
    ...
    output_container = av.open(
        tmp_output_video_path, mode="w",
        options={
            "movflags": "faststart",
            # Normalize timescale to 90000 so that timescale/fps is always an integer,
            # eliminating floating-point drift when accumulating from_timestamp across episodes.
            # (e.g. timescale=12800, fps=30 → 426.67 ticks/frame → cumulative error per episode)
