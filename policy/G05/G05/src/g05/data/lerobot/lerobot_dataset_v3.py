...
def write_image(image: np.ndarray | PIL.Image.Image, path: str | Path) -> None:
    """Write an RGB frame to the temporary episode image directory."""
    path = Path(path)
...
class LeRobotDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        if self.episodes is None:
            ...
            self.episodes = [int(ep_idx) for ep_idx in self.episodes]

        # episode_data_index is local to the loaded HF/Arrow table. When a subset
        # of episodes is loaded, global dataset_from/to indices from metadata are
        # not valid row positions in the filtered table.
        from_list = []
        to_list = []
        self._local_episode_data_index = {}
        local_from = 0
        for ep_idx in self.episodes:
            ep = self.meta.episodes[ep_idx]
            length = int(ep["dataset_to_index"]) - int(ep["dataset_from_index"])
            local_to = local_from + length
            from_list.append(local_from)
            to_list.append(local_to)
            self._local_episode_data_index[ep_idx] = (local_from, local_to)
            local_from = local_to
    ...
    def load_hf_dataset(self) -> datasets.Dataset:
        ...
        columns = self._episode_read_columns()
        features_map = {key: ft for key, ft in self.features.items() if key in columns}
    ...
    def num_frames(self) -> int:
        ...
        """
        if self.episodes is not None and self._np_columns is not None:
            first_col = next(iter(self._np_columns.values()), None)
    ...
    def _get_query_indices(self, idx: int, ep_idx: int) -> tuple[dict[str, list[int | bool]]]:
        ep_start, ep_end = self._local_episode_data_index.get(ep_idx, (None, None))
        if ep_start is None:
            ep = self.meta.episodes[ep_idx]
    ...
    def _get_query_timestamps(
        ...
        for key in self.meta.video_keys:
            if query_indices is not None and key in query_indices:
                if self._np_columns is not None and "timestamp" in self._np_columns:
                    timestamps = self._np_columns["timestamp"][query_indices[key]]
                    query_timestamps[key] = [
                        float(ts[0] if getattr(ts, "ndim", 0) > 0 else ts)
    ...
    def save_episode(self, episode_data: dict | None = None) -> None:
        ...
        task_indices = np.array([self.meta.get_task_index(task) for task in tasks])
        episode_buffer["coarse_task_index"] = task_indices.copy()
        episode_buffer["task_index"] = task_indices
        episode_buffer["coarse_quality_index"] = task_indices.copy()
        episode_buffer["quality_index"] = task_indices.copy()
