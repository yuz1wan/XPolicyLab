...
def handle_resize(signum, frame):
    """Signal handler for terminal resize to refresh tqdm progress bars.
...
def set_global_seed(seed: int, get_worker_init_fn: bool = False) -> Optional[Callable[[int], None]]:
    ...
    assert np.iinfo(np.uint32).min < seed < np.iinfo(np.uint32).max, (
    ...
    # Device-specific seeding: offset by global rank so each GPU gets a different seed
    global_rank = int(os.environ.get("RANK", os.environ.get("LOCAL_RANK", 0)))
    device_seed = seed + global_rank
...
def worker_init_function(worker_id: int) -> None:
    ...
    if torch.distributed.is_available() and torch.distributed.is_initialized():
        ...
        global_rank = int(
            os.environ.get(
                "RANK", os.environ.get("SLURM_PROCID", os.environ.get("LOCAL_RANK", "0"))
            )
        )
