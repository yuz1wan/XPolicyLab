...
class baseframework(PreTrainedModel):
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        # Auto-detect state_placement mismatch: if the checkpoint has condition-side
        # keys (obs_proj._obs_proc) but the model expects noise-side keys
        # (state_noise_proj), rebuild with the correct config. This handles legacy
        # ckpts whose config.full.yaml says "noise" but were actually trained as "condition".
        if hasattr(model_config, "framework") and hasattr(model_config.framework, "action_model"):
            am_cfg = model_config.framework.action_model
            if hasattr(am_cfg, "state_placement"):
                model_keys = set(FrameworkModel.state_dict().keys())
                ckpt_keys = set(model_state_dict.keys())
                has_condition_keys = any("obs_proj._obs_proc" in k for k in ckpt_keys)
                expects_noise_keys = any("state_noise_proj" in k for k in model_keys)
                if has_condition_keys and expects_noise_keys:
                    am_cfg.state_placement = "condition"
                    logger.info("Detected condition-side keys in ckpt; rebuilding model with state_placement=condition")
                    FrameworkModel = build_framework(cfg=model_config)
                    FrameworkModel.norm_stats = norm_stats
