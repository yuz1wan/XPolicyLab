...
XPolicyLab adaptation: encoded RoboTwin images are decoded through the shared
``decode_image_bit`` helper and remain RGB until the OpenCV VideoWriter boundary.
"""
...
class RobotWinConverter:
    ...
    def decode_compressed_image(self, compressed_data: bytes) -> Optional[np.ndarray]:
        ...
        """
        try:
            return decode_image_bit(compressed_data)
        except Exception as e:
            ...
            for i in range(num_frames):
                try:
                    ...
                    if combined_frame is not None:
                        ...
                        # OpenCV VideoWriter is the only BGR boundary. All decoded
                        # and transformed frames remain RGB before this point.
                        combined_frame_bgr = cv2.cvtColor(combined_frame, cv2.COLOR_RGB2BGR)
                        
                        # Ensure correct data type and memory layout
                        if combined_frame_bgr.dtype != np.uint8:
                            combined_frame_bgr = combined_frame_bgr.astype(np.uint8)
                        
                        if not combined_frame_bgr.flags['C_CONTIGUOUS']:
                            combined_frame_bgr = np.ascontiguousarray(combined_frame_bgr)
