...
class WanVideoModel(nn.Module):
    ...
    def decode_video(self, video_latents: torch.Tensor) -> torch.Tensor:
        ...
        """
        with torch.no_grad():
            # print(f"[DEBUG] decode_video input: {video_latents.shape}")
            video_pixels = []
            for i in range(video_latents.shape[0]):
                # print(f"[DEBUG] VAE decode input [{i}]: {video_latents[i].shape}")
                pixels = self.vae.decode([video_latents[i]])[0]
                # print(f"[DEBUG] VAE decode output [{i}]: {pixels.shape}")
                video_pixels.append(pixels)
            result = torch.stack(video_pixels, dim=0)
            # print(f"[DEBUG] decode_video final output: {result.shape}")
            return result
