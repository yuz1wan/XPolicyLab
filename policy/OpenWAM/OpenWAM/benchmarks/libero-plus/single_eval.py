#!/usr/bin/env python3
"""Evaluate a canonical native-action LIBERO-plus checkpoint through the OpenWAM server.
...
def _repo_root() -> Path:
    ...
    if not root.is_dir():
        raise SystemExit(f"LIBERO_PLUS_PATH does not point to a LIBERO-plus repo: {root}")
    return root
...
def _write_libero_config() -> None:
    ...
    config_root = Path(os.environ.get("LIBERO_PLUS_CONFIG_ROOT", Path.home() / ".libero-openwam-plus")).expanduser()
...
def run_eval(cfg: dict) -> int:
    if str(cfg.get("action_mode", "")).strip().lower() != "eef":
        raise ValueError("LIBERO-plus runner requires action_mode: eef")
    if cfg.get("rng_mode", "official_global") != "official_global":
        raise ValueError("LIBERO-plus runner requires rng_mode: official_global")
    _write_libero_config()

    seed = int(cfg.get("seed", 10000))
    _control_seed(seed)
