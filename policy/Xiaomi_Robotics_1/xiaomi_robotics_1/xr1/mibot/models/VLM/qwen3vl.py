...
SCORE_ID = 151669
STATE_ID = 151670
ACTION_START_ID = STATE_ID + 1
ACTION_END_ID = ACTION_START_ID + 60
...
Qwen3VLTextRMSNorm = LigerRMSNorm
...
class Qwen3VLModel(Qwen3VLPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.action_embed = nn.Embedding(ACTION_END_ID - ACTION_START_ID, config.text_config.hidden_size)
        self.score_embed = nn.Embedding(1, config.text_config.hidden_size)
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if input_ids is not None:
            state_mask = input_ids == STATE_ID
            if state_mask.any():
                if state_embeds is None:
                    raise ValueError("State tokens require state_embeds")
                expanded_state_mask = state_mask.unsqueeze(-1).expand_as(inputs_embeds)
                inputs_embeds = inputs_embeds.masked_scatter(expanded_state_mask, state_embeds)

            action_mask = (input_ids >= ACTION_START_ID) & (input_ids < ACTION_END_ID)
            if action_mask.any():
                action_embeds = self.action_embed(input_ids[action_mask] - ACTION_START_ID)
                expanded_action_mask = action_mask.unsqueeze(-1).expand_as(inputs_embeds)
                inputs_embeds = inputs_embeds.masked_scatter(expanded_action_mask, action_embeds)

            score_mask = input_ids == SCORE_ID
            if score_mask.any():
                score_embeds = self.score_embed(torch.zeros_like(input_ids[score_mask]))
                expanded_score_mask = score_mask.unsqueeze(-1).expand_as(inputs_embeds)
                inputs_embeds = inputs_embeds.masked_scatter(expanded_score_mask, score_embeds)
